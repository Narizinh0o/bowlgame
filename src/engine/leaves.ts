...
/**
 * «Дикие» ливы — редкая жесть, примерно 1 игра из 50–100.
 * ТОЛЬКО физически возможные расстановки: сбитые кегли должны объясняться траекторией
 * шара и пин-экшном. Ливы типа 7-8-9-10 (стоит весь задний ряд при сбитых передних)
 * невозможны и удалены по фидбеку Александра.
 */
export const WILDS: LeaveDef[] = [
  { pins: [1, 2, 4, 7, 10], w: 3, convP: 0.08 }, // монстр-вошаут: левый столб целиком + 10
  { pins: [1, 3, 6, 7, 10], w: 2, convP: 0.05 }, // голова + правый столб + одинокая 7
  { pins: [4, 6, 7, 9, 10], w: 1, convP: 0.02 }, // «большая пятёрка» — big four с довеском
]
